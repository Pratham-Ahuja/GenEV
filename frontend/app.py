...
def _inject_hash_reader():
    ...
    token_hash + type (recovery or signup) into query params
    ...
    components.html("""
        ...
        (function() {
            ...
            const token_hash = params.get('token_hash') || params.get('access_token');
            const token_type = params.get('type');

            if (token_hash && (token_type === 'recovery' || token_type === 'signup')) {
...
def _check_recovery_token() -> tuple[str | None, str | None]:
    ...
    Returns (token_hash, token_type) or (None, None).
    ...
    try:
        ...
        # Support both token_hash (new) and access_token (legacy)
        token_hash = params.get("token_hash", "") or params.get("access_token", "")

        if token_hash and token_type in ("recovery", "signup"):
            if token_type == "recovery":
                st.session_state["recovery_token"] = token_hash
...
def main():
    ...
    token_hash, token_type = _check_recovery_token()

    if token_hash and token_type == "recovery":
        render_reset_password_page(token_hash)
        return

    if token_hash and token_type == "signup":
